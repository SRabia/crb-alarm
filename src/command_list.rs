...
impl FromIterator<(Status, &'static str, &'static str)> for CmdList {
    fn from_iter<I: IntoIterator<Item = (Status, &'static str, &'static str)>>(iter: I) -> Self {
        let items = iter
            .into_iter()
            .map(|(status, todo, info)| CmdItem::new(status, todo, info))
            .collect();
...
impl Widget for &mut Cmd {
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        Cmd::render_header(header_area, buf);
        Cmd::render_footer(footer_area, buf);
...
impl From<&CmdItem> for ListItem<'_> {
    fn from(value: &CmdItem) -> Self {
